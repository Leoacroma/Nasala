...
@section('content')
    <div class="container add-news-bg">
        <div class="col-12">
            <h3 class="font-dangrek">វគ្គសិក្សា</h3>
            <form method="POST" action="{{ route('admin.enroll.update', $enroll->id) }}">
              @method('Put')
              @csrf
                <div class="mb-3">
                  <label for="exampleInputEmail1" class="form-label font-Hanuman-bold">ចំណងជើងជាភាសាខ្មែរ</label>
                  <input type="text" name="title_enroll_kh" value="{{ $enroll->title_enroll_kh }}" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" required>
                </div>
                <div class="mb-3">
                  <label for="exampleInputPassword1" class="form-label font-Hanuman-bold">ចំណងជើងជាអង់គ្លេស</label>
                  <input type="text" name="title_enroll_eng"  value="{{ $enroll->title_enroll_eng }}" class="form-control" id="exampleInputPassword1" required>
                </div>
                  <div class="mb-3">
